...
// import '../profile/screens/enquiry_screen.dart';
...
//                                                         children: [
//                                                             Text(
//                                                                 context.tr(_profileStatusMessage),
//                                                                 style: TextStyle(
...
//                                                         children: [
//                                                          ?ApiService.IsBebugMode == true?   ListTile(
...
//                                                         title: context.tr('privacy_policy'),
//                                                         endpoint: '/privacy-policy?type=vendor'
//                                                     )
//                                                 )
//                                             );
//                                         }
...
//                                                     builder: (context) => PolicyContentScreen(
//                                                         title: context.tr('terms_conditions'),
//                                                         endpoint: '/terms-conditions?type=vendor'
//                                                     )
//                                                 )
//                                             );
//                                         }
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      if (token != null) {
        ...
        if (res["success"] == true && res["data"] != null) {
          await _saveUserData(Map<String, dynamic>.from(res["data"]));
        } else {
          await _loadLocalData();
        }
      } else {
  ...
  Future<void> _saveUserData(Map<String, dynamic> userData) async {
    ...
    if (mounted) {
      setState(() {
        ...
        _profilePhotoUrl = userData["profile_photo"]?.toString();
        _statusId = int.tryParse(userData["status_id"]?.toString() ?? '0') ?? 0;
      });
    }

    debugPrint("✅ User data saved and state updated");
  }

  bool get _isVerified => _statusId == 1 && _email.isNotEmpty;
  bool get _shouldShowPendingScreen => !_isVerified;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _shouldShowPendingScreen
          ? PendingApprovalScreen(
        userName: _userName,
        status: _statusId.toString(),
                      ...
                      showModalBottomSheet(
                        ...
                        builder: (context) {
                          return Column(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              if (ApiService.IsBebugMode == true)
                                ListTile(
                                  title: const Text('English'),
                                  onTap: () {
                                    context.read<TranslateProvider>().setLocale('en');
